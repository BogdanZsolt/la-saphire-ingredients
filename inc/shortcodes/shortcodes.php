...
function lasaphire_ingredients_get_list(){
	...
	$return_html .= '
			<ul class="clearfix glassmorph row px-3 mb-3">
				<li>
					<a class="active" title="' . esc_html__('View all components', 'ls-ingredients') . '" data-filter="*">
				'	.	__( 'All', 'ls-ingredients' ) . '
					</a>
				</li>';
	...
	foreach( range( 'A', 'Z' ) as $letter ){
		$return_html .= '
				<li>
					<a class="disabled" title="' . esc_html__('View the component that begins with a letter', 'ls-ingredients') . ' ' . $letter .'" data-filter="cat-' . strtolower($letter) . '">'
						. $letter .
					'</a>
				</li>';
	}
	...
	if( !empty( $query->posts ) ) {
		foreach( $query->posts as $post ){
			...
			$return_html .= '<div class="item-wrapper glassmorph row mb-3 p-3" style="display: flex">
				...
				'</div>
				<div class="col-md-2 col-12">
					<h6>' . esc_html__('Contain', 'ls-ingredients') . ':</h6>
						<ul class="found-in">';
